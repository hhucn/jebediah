...
(defaction echo_name [request-body]
  (simple-speech-response "Hello " (get-in request-body [:result :parameters :given-name])))

(defaction how_long [request-body]
  (simple-speech-response "The word is " (count (get-in request-body [:result :parameters :word])) " characters long."))
